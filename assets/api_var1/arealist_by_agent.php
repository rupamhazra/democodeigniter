 <?php
  ...
  if(!empty($rawDataArray) && $rawDataArray['#serviceName'] == 'arealistbyagent'){
          $serviceName=$rawDataArray['#arealistbyagent'];
		  $agentid=$rawDataArray['agency_id'];
 		  $response['details']= array();
		  
		/*  echo "SELECT * from employee where 1 and `emp_id`='".$emp_id."'";
		  die;*/
		  
		 $qcheck=mysql_query("SELECT id,area_name,area_address,pincode FROM area WHERE d_status='0' and agency_id='".$agentid."'");
