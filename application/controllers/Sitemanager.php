...
class Sitemanager extends CI_Controller {
	...
	{
		$data=array();
		$data['metaInfo']=$this->getMetaDetails(94);
	...
	public function contact(){
		$data=array();
		$data['metaInfo']=$this->getMetaDetails(95);
		$this->template->load('default','front/contact',$data);
	}
	//contact-us save and validate
	public function saveContactForm(){
		$this->_validateContactForm();
		...
		$msg='<body>
					...
					try {
					    ...
					    $mail->addAddress('sales@shyamfuture.com', 'Sales');     // Add a recipient
	...
	private function _validateContactForm(){
		$data = array();
	...
	//home page save,validate and send mail
	public function saveContact(){
		...
		$msg='<body>
					...
					try {
					    ...
					    //Recipients
					    $mail->setFrom('noreply@shyamfuture.com', 'No reply');
					    $mail->addAddress('shail@shyamfuture.com', 'Saby');     // Add a recipient
	...
	public function saveSubscription()
	{
		...
		$data['status'] = TRUE;
         ...
         $result=$this->sitemodel->get_row('subscription','email',$this->input->post('subscriptionemail'));
        if($result){
            $data['inputerror'][] = 'subscriptionemail';
			$data['error_string'][] = 'You have already subscribed';
			$data['status'] = FALSE;
        }
		
		if (!filter_var($this->input->post('subscriptionemail'), FILTER_VALIDATE_EMAIL)) {
			 $data['inputerror'][] = 'subscriptionemail';
			 $data['error_string'][] = 'Invalid email format';
			 $data['status'] = FALSE;
	 	}
		if($this->input->post('subscriptionemail') == '')
		{
			$data['inputerror'][] = 'subscriptionemail';
			$data['error_string'][] = 'Email is required';
			$data['status'] = FALSE;
		...
		$dataSave['email']=$this->input->post('subscriptionemail');
        $dataSave['ipaddress']=getenv('remote_addr');
		//$dataSave['ipaddress']=$_SERVER['REMOTE_ADDR'];
		$dataSave['created_at']=date('Y-m-d H:i:s');
        
        $dataSave=$this->security->xss_clean($dataSave);
		$this->sitemodel->save_Subscription($dataSave);
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="8">
		<tr>
		<td width="83%" align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">Dear Subscriber</span></td>
		</tr>

		<tr>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">Your email id is subscribed</span></td>
		</tr>
		</table>
		...
		try {
		    ...
		    //Recipients
		    $mail->setFrom('noreply@shyamfuture.com', 'No reply');
		    $mail->addAddress($this->input->post('subscriptionemail'), 'User');     // Add a recipient

			 	$mail->addCC('sales@shyamfuture.com');
		    //Content
		    $mail->isHTML(true);                                  // Set email format to HTML
		...
		echo json_encode(array('status'=>TRUE,'test'=> $dataSave['ipaddress']));
	}
	
	public function saveContactwithoutCaptcha(){
		$this->_validateContactWithoutCaptch();
		$data['name']=$this->input->post('inputNameCustom');
		$data['email']=$this->input->post('inputEmailCustom');
		$data['phone']=$this->input->post('inputPhoneCustom');
		$data['company']=$this->input->post('inputCompanyCustom');
		$data['message']=$this->input->post('inputMsgCustom');
		$data['ip_address']=getenv('REMOTE_ADDR');
		$data['contact_date']=date('Y-m-d H:i:s');
		$msg='<body>
		<table width="600px" align="center" border="0" cellspacing="1" cellpadding="5" style="border: groove 1px #3183be;">
		...
		<tr>
		<td width="17%" align="left" valign="top" bgcolor="#FFFFFF"><span class="style22"> Name :</span></td>
		<td width="83%" align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">'.$this->input->post('inputNameCustom').'</span></td>
		</tr>
		<tr>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style22">E-Mail :</span></td>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">'.$this->input->post('inputEmailCustom').'</span></td>
		</tr>

		<tr>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style22">Phone :</span></td>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">'.$this->input->post('inputPhoneCustom').'</span></td>
		</tr>


		<tr>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style22">Message:</span></td>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">'.$this->input->post('inputMsgCustom').'</span></td>
		</tr>
		<tr>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style22">IP Address:</span></td>
		<td align="left" valign="top" bgcolor="#FFFFFF"><span class="style26">'.$_SERVER['REMOTE_ADDR'].'</span></td>
	...
	private function _validateContactWithoutCaptch(){
		...
		$data['inputerror'] = array();
		$data['status'] = TRUE;
		if($this->input->post('inputNameCustom')==''){
			$data['inputerror'][] = 'inputNameCustom';
			$data['error_string'][] = 'Name is required.';
			$data['status'] = FALSE;
		}
		if (!filter_var($this->input->post('inputEmailCustom'), FILTER_VALIDATE_EMAIL)) {
			 $data['inputerror'][] = 'inputEmailCustom';
			 $data['error_string'][] = 'Invalid email format';
			 $data['status'] = FALSE;
	 }
		if($this->input->post('inputEmailCustom') == '')
		{
			$data['inputerror'][] = 'inputEmailCustom';
			$data['error_string'][] = 'Email is required';
			$data['status'] = FALSE;
		}
		if(!is_numeric($this->input->post('inputPhoneCustom')))
		{
			$data['inputerror'][] = 'inputPhoneCustom';
			$data['error_string'][] = 'Only numbers allowed';
			$data['status'] = FALSE;
		}
		if($this->input->post('inputPhoneCustom') == '')
		{
			$data['inputerror'][] = 'inputPhoneCustom';
			$data['error_string'][] = 'Phone is required';
			$data['status'] = FALSE;
		}
		if($this->input->post('inputMsgCustom')==''){
			$data['inputerror'][] = 'inputMsgCustom';
			$data['error_string'][] = 'Message is required.';
			$data['status'] = FALSE;
		}
...
public function career(){
	...
	public function showPage(){
		...
		if($url=='android-app-development'){
			...
			//http://api.ipstack.com/203.163.246.157?access_key=1a300dec54300798bdcb496c638991be
			$details = json_decode(file_get_contents('http://api.ipstack.com/'.$ip.'?access_key=1a300dec54300798bdcb496c638991be'));
			//$details = json_decode(file_get_contents('http://freegeoip.net/json/'.$ip));
			$trackData['location']=$details->country_name.",".$details->city;
		...
		else if($url=='web-design-development'){
			$trackData['ip_address']=$_SERVER['REMOTE_ADDR'];
			$trackData['open_time']=Date('Y-m-d H:i:s');
			$trackData['page_name']="web-design-development";
			$ip=$_SERVER['REMOTE_ADDR'];
			$details = json_decode(file_get_contents('http://api.ipstack.com/'.$ip.'?access_key=1a300dec54300798bdcb496c638991be'));
			$trackData['location']=$details->country_name.",".$details->city;
			$this->sitemodel->track_page($trackData);
		...
		$data['banner']=$data['page']['banner_path'];
		$data['banner_content']=$data['page']['banner_content'];
	...
	public function blogList(){
		...
		$this->template->load('default','front/blog_list',$data);
	}
	...
	public function blogDetails(){
		...
		$blog = $this->blogmodel->get_blog_by_url($url);
		$data['blogCategoryList']=$this->blogmodel->getParentList();
		$blog_id = $blog['blog_post_id'];
		$data['blog'] = $blog;
		$data['metaInfo']=$this->getBlogMetaDetails($blog_id);
		//echo $url; die;
		//print_r($data['blog']); die;
		if(count($data['blog'])>0)
//		$this->load->view('front/blogdetails',$data);
        $this->template->load('default','front/blogdetails',$data);
		else
			redirect(base_url('404'));
	}
	...
	public function getBlogMetaDetails($blogId=false) {
		$metaString = "";
		//$blogId = 10;
		 if($blogId){
		 		$metaVal =  $this->sitemodel->get_meta_by_id($blogId);

					if( $metaVal['blog_title'])
						 $metaString = $metaString."<title>".$metaVal['blog_title']."</title>".PHP_EOL;
					if( $metaVal['meta_description'])
						$metaString = $metaString.'<meta name="description" content="'.$metaVal["meta_description"].'" />'.PHP_EOL;
					if( $metaVal['meta_keyword'])
						$metaString = $metaString.'<meta name="keywords" content="'.$metaVal['meta_keyword'].'" />'.PHP_EOL;
